...
const navItems = [
  ...
  { id: 'decisions', label: 'Decision Journal', icon: NotebookPen },
...
function BottomNav({ page, setPage, onMore }) {
  ...
  return (
    <nav className="bottom-nav" aria-label="Mobile navigation">
      ...
      <button className={['home', 'mood', 'travel', 'content', 'relationships', 'insights', 'decisions', 'assistant', 'timeline', 'settings'].includes(page) ? 'active' : ''} onClick={onMore}><Sparkles size={19} /><span>More</span></button>
    </nav>
  );
}
  ...
  return (
    <>
      ...
      <Card className="morning-brief-card">
        <div className="brief-intro"><Pill tone="blue">Morning brief · {currentCity}</Pill><h2>Good morning, {profile?.display_name || 'there'}.</h2><p>{executiveBrief?.today?.highest_leverage_action || briefing?.summary || (data ? 'Record tasks, habits, objectives, finance, travel, and check-ins to generate a data-cited operating brief.' : 'Loading your current operating picture…')}</p><div className="brief-status"><span><small>Current risk</small><strong>{risks.some((item) => item.severity === 'high' || item.level === 'high') ? 'HIGH' : risks.some((item) => item.severity === 'medium' || item.level === 'medium') ? 'MEDIUM' : 'LOW'}</strong></span><span><small>Executive score</small><strong>{executiveScore?.operating_score ?? data?.executionReadiness ?? '—'}</strong></span><span><small>Weekly trend</small><strong>{data?.weeklyTrend?.direction || 'flat'} {data?.weeklyTrend?.delta ? `${data.weeklyTrend.delta > 0 ? '+' : ''}${data.weeklyTrend.delta}` : ''}</strong></span></div></div>
...
function GoalsPage() {
  ...
  return (
    <><PageHeader eyebrow="Life Vision → Objectives → Goals → Tasks" title={<>Objectives & execution<span className="title-dot">.</span></>} copy="Life Vision is the long-term direction. Objectives are 3-12 month outcomes. Goals are measurable milestones. Tasks are specific actions." action={<div className="assistant-actions"><button className="text-button" title="Task: a specific action item" onClick={() => setAdding('task')}><Plus size={17} /> Task</button><button className="subtle-button" title="Goal: a measurable milestone supporting an objective" onClick={() => setAdding('goal')}><Plus size={17} /> Goal</button><button className="primary-button" title="Objective: a major 3-12 month outcome" onClick={() => setAdding('objective')}><Plus size={17} /> Objective</button></div>} />
...
function DecisionJournalPage() {
  ...
  const [notice, setNotice] = useState('');
  const load = () => fetch('/api/decisions').then((response) => response.json()).then((result) => setDecisions(result.data || []));
  useEffect(() => { load().catch(() => {}); }, []);
  const saveDecision = async (event) => { event.preventDefault(); const form = event.currentTarget; const body = Object.fromEntries(new FormData(form)); body.decision = body.title; body.reasoning = body.reason_chosen || body.context || 'No reasoning recorded.'; const response = await fetch('/api/decisions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const result = await response.json(); setNotice(response.ok ? 'Decision logged.' : result.error); if (response.ok) { form.reset(); load(); } };
  const reviewDecision = async (decision) => { const outcome = window.prompt('Actual outcome', decision.outcome || decision.actual_outcome || ''); if (outcome == null) return; const lessons = window.prompt('Lessons learned', decision.lessons_learned || '') || ''; const quality_score = Number(window.prompt('Decision quality score (0-100)', decision.quality_score ?? 70)); const response = await fetch(`/api/decisions/${decision.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ outcome, actual_outcome: outcome, lessons_learned: lessons, quality_score, reviewed_at: new Date().toISOString() }) }); if (response.ok) load(); };
  const deleteDecision = async (decision) => { if (!window.confirm(`Delete ${decision.title || decision.decision}?`)) return; const response = await fetch(`/api/decisions/${decision.id}`, { method: 'DELETE' }); if (response.ok) load(); };
  return <><PageHeader eyebrow="Decision Journal" title={<>Decision quality ledger<span className="title-dot">.</span></>} copy="Track major life decisions, expected outcomes, confidence, review dates, actual outcomes, and lessons learned." />
    {notice && <div className="system-notice">{notice}</div>}
    <Card><SectionTitle title="Log decision" meta="Capture reasoning before outcome bias" /><form className="wellbeing-form" onSubmit={saveDecision}><label>Title<input name="title" required /></label><label>Date<input name="decision_date" type="date" defaultValue={new Date().toISOString().slice(0, 10)} /></label><label className="full-field">Context<textarea name="context" required /></label><label className="full-field">Options considered<textarea name="options_considered" placeholder="One option per line" /></label><label className="full-field">Reason chosen<textarea name="reason_chosen" required /></label><label className="full-field">Expected outcome<textarea name="expected_outcome" required /></label><label>Confidence<input name="confidence" type="number" min="0" max="100" /></label><label>Review date<input name="review_date" type="date" /></label><button className="primary-button full-field">Save decision</button></form></Card>
    <Card><SectionTitle title="Decision log" meta={`${decisions.length} decisions`} /><div className="signal-list">{decisions.length ? decisions.map((decision) => <div key={decision.id}><b>{decision.title || decision.decision}</b><span>{decision.expected_outcome}</span><Pill tone={decision.reviewed_at ? 'green' : decision.review_date && new Date(`${decision.review_date}T23:59:59`).getTime() < Date.now() ? 'amber' : 'neutral'}>{decision.reviewed_at ? `Quality ${decision.quality_score ?? '—'}` : decision.review_date || 'No review date'}</Pill><button className="text-button" onClick={() => reviewDecision(decision)}>Review</button><button className="text-button" onClick={() => deleteDecision(decision)}>Delete</button></div>) : <p className="plan-empty">No decisions logged yet.</p>}</div></Card>
  </>;
}
...
function MoodPage() {
  ...
  const saveProfile = async (event) => { event.preventDefault(); const form = event.currentTarget; const body = Object.fromEntries(new FormData(form)); Object.keys(body).forEach((key) => { if (body[key] === '') delete body[key]; }); const url = wellbeing.profile ? `/api/health_profiles/${wellbeing.profile.id}` : '/api/health_profiles'; const response = await fetch(url, { method: wellbeing.profile ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); setSaved(response.ok ? 'Health profile saved.' : (await response.json()).error); if (response.ok) load(); };
  ...
  return (
    ...
    {view === 'profile' && <div className="two-col-layout wellbeing-layout"><Card><SectionTitle title="Health profile" meta="Body composition and diet profile" /><form className="wellbeing-form" onSubmit={saveProfile}><label>Height<input name="height" type="number" step=".01" defaultValue={wellbeing.profile?.height || ''} /></label><label>Weight<input name="weight" type="number" step=".01" defaultValue={wellbeing.profile?.weight || ''} /></label><label>Goal weight<input name="goal_weight" type="number" step=".01" defaultValue={wellbeing.profile?.goal_weight || ''} /></label><label>Body fat %<input name="body_fat_pct" type="number" step=".01" defaultValue={wellbeing.profile?.body_fat_pct || ''} /></label><label>Waist<input name="waist" type="number" step=".01" defaultValue={wellbeing.profile?.waist || ''} /></label><label>Neck<input name="neck" type="number" step=".01" defaultValue={wellbeing.profile?.neck || ''} /></label><label>Diet profile<select name="diet_profile" defaultValue={wellbeing.profile?.diet_profile || 'Custom'}>{['High Protein','Mediterranean','Vegetarian','Vegan','Keto','Custom'].map((item) => <option key={item}>{item}</option>)}</select></label><label className="full-field">Custom diet<input name="custom_diet" defaultValue={wellbeing.profile?.custom_diet || ''} /></label><button className="primary-button full-field">Save health profile</button></form></Card>
...
function AssistantPage() {
  ...
  const load = () => Promise.all([fetch('/api/status').then((response) => response.json()), fetch('/api/dashboard').then((response) => response.json()), fetch('/api/decisions').then((response) => response.json())]).then(([nextStatus, nextDashboard, nextDecisions]) => { setStatus(nextStatus); setDashboard(nextDashboard); setDecisions(nextDecisions.data || []); });
  ...
  const saveDecision = async (event) => {
    ...
    const response = await fetch('/api/decisions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.fromEntries(new FormData(form))) });
    const result = await response.json();
    if (!response.ok) { setMessages((current) => [...current, { from: 'ai', text: `Decision log failed: ${result.error}` }]); return; }
    form.reset();
    await load();
  };
  const reviewDecision = async (decision) => {
    const actual_outcome = window.prompt('Actual outcome', decision.actual_outcome || '');
    if (!actual_outcome) return;
    const quality_score = Number(window.prompt('Decision quality score (0-100)', decision.quality_score ?? 70));
    const response = await fetch(`/api/decisions/${decision.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ actual_outcome, quality_score }) });
    if (response.ok) load();
  };
  ...
  return (
    ...
    <Card><SectionTitle title="Decision journal" meta={`${decisions.length} logged`} /><form className="wellbeing-form" onSubmit={saveDecision}><label className="full-field">Decision<textarea name="decision" required /></label><label className="full-field">Reasoning<textarea name="reasoning" required /></label><label className="full-field">Expected outcome<textarea name="expected_outcome" required /></label><label>Review date<input name="review_date" type="date" /></label><button className="primary-button full-field">Log decision</button></form><div className="signal-list">{decisions.slice(0, 5).map((decision) => <div key={decision.id}><b>{decision.decision}</b><span>{decision.expected_outcome}</span><Pill tone={decision.reviewed_at ? 'green' : 'neutral'}>{decision.reviewed_at ? `Quality ${decision.quality_score ?? '—'}` : decision.review_date || 'No review date'}</Pill><button className="text-button" onClick={() => reviewDecision(decision)}>Review</button></div>)}</div></Card></div></div></>
  );
}
...
function App() {
  ...
  const Page = useMemo(() => ({ dashboard: <Dashboard habits={habits} toggleHabit={toggleHabit} />, goals: <GoalsPage />, habits: <HabitsPage habits={habits} toggleHabit={toggleHabit} reloadHabits={loadHabits} />, money: <MoneyPage />, home: <HomeBasePage setPage={setPage} />, mood: <MoodPage />, travel: <TravelPage />, content: <ContentPage />, relationships: <RelationshipsPage />, insights: <InsightsPage />, decisions: <DecisionJournalPage />, assistant: <AssistantPage />, timeline: <TimelinePage />, settings: <SettingsPage />, school: <SchoolPage /> })[page], [page, habits]);
