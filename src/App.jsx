...
function Topbar({ page, appName, setPage }) {
  ...
  return (
    <div className="topbar">
      ...
      <div className="top-actions">
        <button className="icon-button" aria-label="Search insights" onClick={() => setPage('insights')}><Search size={18} /></button>
        <button className="icon-button notification" aria-label="Open Chief of Staff" onClick={() => setPage('assistant')}><Bell size={18} /><i /></button>
...
function GoalsPage() {
  ...
  return (
    ...
    <Card className="planning-help-card"><div className="signal-list"><div><b>Vision</b><span>Long-term direction, such as Graduate WGU.</span></div><div><b>Objective</b><span>Major outcome, such as Graduate by December 2026.</span></div><div><b>Goal</b><span>Measurable milestone, such as Complete 18 courses.</span></div><div><b>Task</b><span>Specific action, such as Finish Chapter 4.</span></div></div></Card>
...
function MoodPage() {
  ...
  return (
    ...
    {view === 'history' && <><Card><SectionTitle title="Check-in trends" meta="Chronological mood, energy, stress, and productivity" /><form className="studio-add-form compact-form" onSubmit={(event) => { event.preventDefault(); load(search, historyFrom, historyTo); }}><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search wins, challenges, avoidance, or priorities" /><input type="date" value={historyFrom} onChange={(event) => setHistoryFrom(event.target.value)} aria-label="From date" /><input type="date" value={historyTo} onChange={(event) => setHistoryTo(event.target.value)} aria-label="To date" /><button className="primary-button">Filter history</button></form><div className="checkin-trends">{['mood', 'energy', 'stress', 'productivity'].map((field) => <div key={field}><span>{field}</span><MiniBars values={wellbeing.trends.slice(-14).map((item) => Number(item[field] || 0) * 10)} tone={field === 'stress' ? 'violet' : 'mint'} /></div>)}</div></Card><Card className="learning-timeline-card"><SectionTitle title="Daily check-in ledger" meta={`${wellbeing.checkins.length} entries`} /><div className="learning-timeline checkin-ledger">{wellbeing.checkins.length ? wellbeing.checkins.map((entry) => <button key={entry.id} onClick={() => setSelected(entry)}><span>{new Date(`${entry.checkin_date}T12:00:00`).toLocaleDateString([], { month: 'short', day: 'numeric' })}</span><i /><strong>{entry.biggest_win || entry.tomorrow_priority || 'Daily check-in'}</strong><Pill tone={entry.productivity >= 8 ? 'green' : entry.stress >= 8 ? 'amber' : 'neutral'}>M{entry.mood} · E{entry.energy} · S{entry.stress} · P{entry.productivity}</Pill></button>) : <p className="plan-empty">No matching check-ins.</p>}</div></Card>{selected && <Card><SectionTitle title={new Date(`${selected.checkin_date}T12:00:00`).toLocaleDateString([], { weekday: 'long', month: 'long', day: 'numeric' })} meta="Past check-in" action={<button className="text-button" onClick={() => setSelected(null)}>Close</button>} /><div className="signal-list"><div><b>Biggest win</b><span>{selected.biggest_win || 'Not recorded'}</span></div><div><b>Biggest challenge</b><span>{selected.biggest_challenge || 'Not recorded'}</span></div><div><b>Avoidance</b><span>{selected.what_was_avoided || 'Not recorded'}</span></div><div><b>Tomorrow priority</b><span>{selected.tomorrow_priority || 'Not recorded'}</span></div><div><b>AI summary</b><span>{selected.ai_summary || 'Not available'}</span></div></div></Card>}</>}
    </>
  );
}
  ...
  return (
    <div className="app-shell" data-theme={profile?.preferences?.theme || 'default'} data-font={profile?.preferences?.font_style || 'default'} data-density={profile?.preferences?.density || 'comfortable'} style={{ '--mint': profile?.preferences?.accent_color || '#a7f3d0' }}>
      ...
      <div className="app-main">
        <Topbar page={page} setPage={setPage} appName={profile?.preferences?.app_name || (profile?.display_name ? `${profile.display_name} OS` : 'You OS')} />
