...
export const entitySchemas = {
  ...
  health_samples: z.object({ sample_type: z.enum(['steps','active_calories','resting_heart_rate','sleep_duration','stand_hours','vo2_max','weight','heart_rate_variability']), value: z.coerce.number(), unit: z.string().trim().min(1).max(30), sampled_at: dateString, source: z.string().trim().max(50).optional(), notes: optionalText }),
  workout_sessions: z.object({ workout_type: z.string().trim().min(1).max(120), started_at: z.string().datetime(), ended_at: dateString, duration_minutes: z.coerce.number().nonnegative().optional().nullable(), active_calories: z.coerce.number().nonnegative().optional().nullable(), distance: z.coerce.number().nonnegative().optional().nullable(), source: z.string().trim().max(50).optional(), notes: optionalText }),
  ...
  calendar_events: z.object({ title: z.string().trim().min(1).max(250), category: z.string().trim().max(50).optional(), starts_at: z.string().datetime(), ends_at: z.string().datetime(), location: optionalText, notes: optionalText, source: z.string().trim().max(50).optional(), external_id: optionalText, confidence_score: z.coerce.number().int().min(0).max(100).optional().nullable(), generated_from: optionalText }),
  ...
  daily_checkins: z.object({ checkin_date: z.string().date().optional(), checked_in_at: dateString, local_date: z.string().date().optional(), timezone: z.string().trim().max(100).optional(), time_of_day: z.enum(['morning','afternoon','evening','night']).optional(), mood: z.coerce.number().int().min(1).max(10), energy: z.coerce.number().int().min(1).max(10), stress: z.coerce.number().int().min(1).max(10), productivity: z.coerce.number().int().min(1).max(10), sleep_hours: z.coerce.number().min(0).max(24).optional().nullable(), biggest_win: optionalText, biggest_challenge: optionalText, what_was_avoided: optionalText, tomorrow_priority: optionalText, ai_summary: optionalText, notes: optionalText }),
  context_events: z.object({ raw_input: z.string().trim().min(1).max(2000), title: z.string().trim().min(1).max(250), event_date: z.string().date(), event_time: z.string().optional().nullable(), category: z.string().trim().max(80).optional(), location: optionalText, notes: optionalText, confidence_score: z.coerce.number().int().min(0).max(100).optional(), needs_confirmation: booleanField.optional() }),
  user_preferences: z.object({ app_name: optionalText, theme: z.enum(['default', 'midnight', 'soft']).optional(), accent_color: z.string().regex(/^#[0-9a-f]{6}$/i).optional(), font_style: z.enum(['default', 'editorial', 'system']).optional(), density: z.enum(['comfortable', 'compact']).optional(), chief_of_staff_tone: z.enum(['gentle', 'executive', 'direct']).optional(), design_preferences: z.record(z.unknown()).optional() }).strict(),
...
export const onboardingSchema = z.object({
  ...
  design: z.object({ app_name: z.string().trim().max(100).optional(), theme: z.enum(['default', 'midnight', 'soft']), accent_color: z.string().regex(/^#[0-9a-f]{6}$/i), font_style: z.enum(['default', 'editorial', 'system']), density: z.enum(['comfortable', 'compact']) }).strict(),
});
