  ...
  return (
    <main className="auth-shell onboarding-shell">
      <section className="card onboarding-card">
        ...
        {step === 1 && <div className="onboarding-fields"><div className="system-notice full-field">Vision is long-term direction. Objectives are 3-12 month outcomes. Goals are measurable milestones. Tasks are specific actions.</div><label className="full-field">Goals<textarea value={form.goals} onChange={(event) => update('goals', event.target.value)} placeholder={'One per line: Goal | Category | Target date\nComplete 18 WGU courses | education | 2026-12-01'} required /></label><label className="full-field">Habits<input value={form.habits} onChange={(event) => update('habits', event.target.value)} placeholder="Meditation, coding, lifting, running" /></label><label className="full-field">Travel plans<textarea value={form.travel_plans} onChange={(event) => update('travel_plans', event.target.value)} placeholder="One per line: Title | City | Country | Arrival | Departure | Budget" /></label></div>}
